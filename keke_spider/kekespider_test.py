...
class KeKeSpider:
    ...
    def _parsing_page(self, html, language="en"):
        try:
            ...
            # 去除文章中script标签
            [i.extract() for i in article.find_all('img')]
            ...
            # # 分段逻辑开始
            # tags = [p for p in article.find_all("p") if str(p.text).strip()]
            # content = []
            # # tag 包含 多个段落 c 中又包含段落信息
            # for c in tags:
            #     start, end = 0, 0  # 两个游标记录 段落开始结束
            #     cs = c.contents
            #     last_index = 0  # 总是记录最后一次的位置
            ...
            #             # 根据br位置索引来分段
            #             pline = cs[start:end]
            #             content.append(" ".join([str(p) for p in pline]))
            #         else:
            #             if i == len(cs)-1:
            #                 pline = cs[last_index:i]
            #                 content.append(" ".join([str(p) for p in pline]))

            # 分段逻辑结束

            content = [str(c).replace("<br/>", "") for c in content]
...
class FoxSpider(KeKeSpider):
    ...
    def _parsing_page(self, html, language="en"):
        try:
            ...
            # 分段逻辑开始
            tags = [p for p in article.find_all("p", attrs={"class": "p"}) if str(p.text).strip()]
            content = []
            # tag 包含 多个段落 c 中又包含段落信息
            for c in tags:
                start, end = 0, 0  # 两个游标记录 段落开始结束
            ...
            content = [str(c).replace("<br/>", "") for c in content]
            content = content[0:-1] if content[-1].count("译文") or content[-1].count("原创") else content
            # 去掉最最后一段js代码 如果有
            if content[-1].count("R("):
                content[-1] = content[-1][:content[-1].index("R(")]
...
#     urls = ['http://www.kekenet.com/broadcast/201905/581905.shtml', ]
#     spider = KeKeSpiderProxy(url=urls[0])
